...
import { IStatusInfo, IUsageChart, IUsageData } from "../../types";
...
export const getCpuUsageData = async (req: Request, res: Response) => {
  try {
    ...
    return res.status(200).json(convertUsageData(data));
  } catch (ex) {
...
const convertUsageData = (data: IUsageData): IUsageChart[] => {
  return data.labels.map((label, index) => ({ name: label, usage: data.data[index] }))
}